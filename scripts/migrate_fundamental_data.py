...
根据聚宽API文档中的valuation估值数据表字段，对齐fundamental_data表结构
...
class FundamentalDataMigrator:
    ...
    def get_current_schema(self) -> List[Tuple[str, str]]:
        """获取当前fundamental_data表的字段结构"""
        try:
            result = self.conn.execute("PRAGMA table_info(fundamental_data)").fetchall()
    ...
    def add_missing_columns(self, missing_fields: List[str]):
        ...
        for field in missing_fields:
            if field in target_schema:
                ...
                try:
                    sql = f"ALTER TABLE fundamental_data ADD COLUMN {field} {field_type}"
    ...
    def create_new_table_if_not_exists(self):
        ...
        try:
            ...
            logger.info("fundamental_data表创建成功或已存在")
        except Exception as e:
    ...
    def backup_table(self) -> str:
        ...
        backup_table_name = f"fundamental_data_backup_{timestamp}"
        ...
        try:
            ...
            if 'fundamental_data' in table_names:
                sql = f"CREATE TABLE {backup_table_name} AS SELECT * FROM fundamental_data"
                self.conn.execute(sql)
                logger.info(f"已备份表为: {backup_table_name}")
                return backup_table_name
            else:
                logger.info("fundamental_data表不存在，无需备份")
                return ""
        except Exception as e:
    ...
    def migrate(self, backup: bool = True):
        """执行迁移"""
        logger.info("开始迁移fundamental_data表结构")
        ...
        try:
            ...
            logger.info("fundamental_data表结构迁移完成")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='基本面数据表字段对齐工具')
    # 获取默认数据库路径
    import sys
    sys.path.append('..')
    from config import get_config
    ...
    migrator = FundamentalDataMigrator(args.db_path)
